                        ...
                        <table id="soldOrdersTable" class="table table-striped-columns table-hover table-border">
                            ...
                            <?php if (!($cache = WarehouseAbstract::getCache('soldOrdersTable'))): ?>
                                ...
                                <?php foreach ($soldOrders as $index => $order): ?>
                                    ...
                                    ?>
                                    <tr>
                                        <td class="<?= !$order->fulfilled_at ? 'bg-warning' : '' ?>"><?= !$order->fulfilled_at ? 'B ' : '' ?><?= str_pad($order->id, 6, '0', STR_PAD_LEFT) ?></td>
                                        <td class="<?= !$order->fulfilled_at ? 'bg-warning' : '' ?>">
                                            ...
                                            (<?= $order->object instanceof WarehouseProduct ? htmlspecialchars($order->object->fnsku) : htmlspecialchars($order->object->parent->name) ?>)
                                        </td>
                                        <td class="<?= !$order->fulfilled_at ? 'bg-warning' : '' ?>"><?= nl2br(htmlspecialchars($order->description)) ?></td>
                                        <td class="<?= !$order->fulfilled_at ? 'bg-warning' : '' ?>">
                                            <strong><?= $logAdd ? $logAdd->username() : '' ?></strong><br>
                                            <?= htmlspecialchars($order->created_at) ?>
                                        </td>
                                        <td class="<?= !$order->fulfilled_at ? 'bg-warning' : '' ?>">
                                            <strong><?= $logFulfil ? $logFulfil->username() : '' ?></strong><br>
                                            <?= htmlspecialchars($order->fulfilled_at) ?>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
