...
function productInfo($product) {
    ...
    return "
    <div id='img{".htmlentities($product->fnsku)."' class='col-12 col-md-3'><img src='{$img}'  class='img-fluid img-thumbnail'></div>
    ...
    foreach($raflar as $raflar_name => $raflar_containers) {
        ...
        foreach($raflar_containers as $container) {
            ...
            if ($product instanceof WarehouseProduct) {
                ...
                $html .= ($container->type === 'Koli' && count($container->getProducts()) > 1) ? ' (Karışık koli)' : '';
            }
            $html .= '</option>';
        }
        $html .= '</optgroup>';
