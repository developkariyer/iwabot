...
function getUserChannelMsgCount($channelId, $userId=null) {
    if ($userId !== null) {
        $stmt = $GLOBALS['pdo']->prepare("SELECT msgcount FROM rawlogstats WHERE channel = :channel_id AND user = :user_id");
        $stmt->execute(['channel_id' => $channelId, 'user_id' => $userId]);
    } else {
        $stmt = $GLOBALS['pdo']->prepare("SELECT SUM(msgcount) FROM rawlogstats WHERE channel = :channel_id");
